...
Dashboard with charts:
  ...
  - Subject comparison
  - Top performers and low-attendance alerts
"""
...
def render_dept_section():
    ...
    with c1:
        ...
        dept = (merged.groupby("department")
                      .agg(total=("student_id","count"),
                           present=("status", lambda x: (x=="Present").sum()))
                      .reset_index())
        dept["rate"] = (dept["present"] / dept["total"] * 100).round(1)
        st.dataframe(dept.rename(columns={"department":"Department","rate":"Rate %"}),
                     use_container_width=True, hide_index=True)

    with c2:
        st.subheader("By Section")
        sec = (rdf.groupby("section")
                  .agg(total=("student_id","count"),
                       present=("status", lambda x: (x=="Present").sum()))
                  .reset_index())
        sec["rate"] = (sec["present"] / sec["total"] * 100).round(1)
        st.dataframe(sec.rename(columns={"section":"Section","rate":"Rate %"}),
                     use_container_width=True, hide_index=True)
...
def render_subjects():
    ...
    c2.dataframe(subj.rename(columns={"subject":"Subject","rate":"Rate %"}),
                 use_container_width=True, hide_index=True)
...
def render_student_performance():
    ...
    with c1:
        ...
        st.dataframe(top.rename(columns={"name":"Name","department":"Dept","rate":"Rate %"}),
                     use_container_width=True, hide_index=True)
    with c2:
        st.markdown("**⚠️ Below 75% attendance**")
        low = perf[perf["rate"] < 75].nsmallest(10, "rate")[["name", "department", "rate"]]
        if low.empty:
            st.success("All students ≥ 75% – well done!")
        else:
            st.dataframe(low.rename(columns={"name":"Name","department":"Dept","rate":"Rate %"}),
                         use_container_width=True, hide_index=True)
...
def render_insights():
    ...
    emoji   = "✅" if rate >= 90 else ("ℹ️" if rate >= 75 else "⚠️")
    st.markdown(f"- {emoji} Overall attendance rate (last 30 days): **{rate}%**")
    ...
    st.markdown(f"- 👥 **{unique}** unique students marked attendance")
...
def main():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["📊 Trends", "🏢 Dept / Section",
                                       "📚 Subjects", "👥 Students"])
    with tab1:  render_trends()
    with tab2:  render_dept_section()
    with tab3:  render_subjects()
    with tab4:  render_student_performance()
